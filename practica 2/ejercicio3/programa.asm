# Prog de prueba para Práctica 2
...
main:lw $s1, 0($zero) 
  ...
  nop
  nop
  addi $s1, $s1, 1 #guarda un 2 en r17
  beq $s1, $s2, salto1 # no salta
  addi $s3, $s3, 1 #guarda un 5 en r19
  nop
  nop
  nop
  nop
  salto1:addi $s2, $s2, 1 #guarda un 2 en r18
  beq $s1, $s2, salto2 # si salta
  addi $s3, $s3, 1 #NO SE EJECUTA (guarda un 6 en r19)
  nop
  nop
  nop
  nop
  salto2:lw $s1, 0($zero) #guarda un 1 en r17
  beq $s1, $s2, salto3 #NO salta
  addi $s3, $s3, 1 #guarda un 6 en r19
  nop
  nop
  nop
  nop
  salto3:lw $s2, 0($zero) #guarda un 1 en r18
  beq $s1, $s2, salto4 #Si salta
  addi $s3, $s3, 1 #NO SE EJECUTA (guarda un 7 en r19)
  nop
  nop
  nop
  nop
  # carga num0 a num5 en los registros 9 a 14
  salto4:lw $t1, 0($zero) # lw $r9, 0($r0)
